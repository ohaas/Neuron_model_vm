...
# AND A IS THE AMPLITUDE
    ...
    def __init__(self, mu, sigma, x1 , A):
        self.y=ny.zeros(360.0)
        x2=ny.arange( 0.0, 360.0, 1 )
        for x in x2:
            self.y[x]= A*25.0*((1.0/(sigma*ny.sqrt(2*ny.pi)))*ny.exp(-(x-mu)**2/(2.0*sigma**2)))+A*25.0*((1.0/(sigma*ny.sqrt(2*ny.pi)))*ny.exp(-(x-(mu-360))**2/(2.0*sigma**2)))
        print self.y[x1]
        pp.plot(x2,self.y)
        pp.xlim(0,360)
